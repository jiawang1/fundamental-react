          ...
          {items.map(item => {
            return (
              <li className="fd-mega-menu__item" key={item.id}>
                {item.link ? (
                  <Link
                    className={`fd-mega-menu__link${this.state.selectedItem === item.id ? ' is-selected' : ''}${item.hasChild ? ' has-child' : ''}${
                ...
                {item.hasChild ? (
                  <ul className="fd-mega-menu__sublist" id={item.id} aria-hidden={!this.state.itemStates[item.id]} aria-expanded={this.state.itemStates[item.id]}>
                    {item.child.map(ch => {
                      return (
                        <li className="fd-mega-menu__subitem" key={ch.id}>
                          {ch.link ? (
                            <Link
                              className={`fd-mega-menu__sublink${this.state.selectedItem === ch.id ? ' is-selected' : ''}`}
                              to={{ pathname: ch.link }}
                              key={ch.id}
