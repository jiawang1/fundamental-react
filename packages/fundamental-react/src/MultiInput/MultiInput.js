...
export class MultiInput extends Component {
    ...
    return data.map((item, index) => (
      <li key={index}>
        <label htmlFor={index + `_${randNum}`} className="fd-menu__item">
          <input type="checkbox" className="fd-checkbox" id={index + `_${randNum}`} value={item} onChange={this.updateSelectedTags} checked={this.isChecked(item)} />
          {item}
        </label>
      </li>
    ));
  ...
  createTags = tags => {
    return this.state.tags.map((tag, index) => (
      <span key={index} className="fd-token" role="button" onClick={this.removeTag}>
        {tag}
      </span>
    ));
  };
  ...
  render() {
    ...
    const inputGroupClassNames = `fd-input-group fd-input-group--after${compact ? ' fd-input-group--compact' : ''}`;
    ...
    return (
      <div className={`fd-multi-input${className ? ' ' + className : ''}`} {...rest}>
        <div className="fd-multi-input-field">
          <div className="fd-popover">
            <div className="fd-popover__control">
              <div className="fd-combobox-control" aria-label="Image label" aria-expanded={this.state.bShowList} aria-haspopup="true">
                <div className={inputGroupClassNames}>
                  <input type="text" className={inputClassNames} placeholder={placeHolder} onClick={this.showHideTagList} />
                  <span
                    className="fd-input-group__addon fd-input-group__addon--after
                            fd-input-group__addon--button"
                  >
                    <button className="fd-button--light sap-icon--navigation-down-arrow" onClick={this.showHideTagList} />
                  </span>
                </div>
              </div>
            </div>
            <div className="fd-popover__body fd-popover__body--no-arrow" aria-hidden={!this.state.bShowList}>
        ...
        {this.state.tags.length > 0 ? <div className="fd-multi-input-tags">{this.createTags()}</div> : ''}
      </div>
    );
  }
}
