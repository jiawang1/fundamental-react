...
export class Pagination extends Component {
  ...
  navigateBack = () => {
    ...
    this.setState(prevState => ({ selectedPage: --prevState.selectedPage }), () => this.props.onClick(this.state.selectedPage));
  };
  ...
  createPaginationLinks = numberOfPages => {
    ...
    const aPages = Array(numberOfPages)
      .fill()
      .map((link, index) => (
        <a key={index} href="#" className="fd-pagination__link" aria-selected={this.state.selectedPage === index + 1} onClick={this.pageClicked}>
          {index + 1}
        </a>
      ));
    return aPages;
  ...
  render() {
    const { itemsTotal, itemsPerPage = 10, displayTotal = true, totalText, className } = this.props;

    // calculate number of pages based on item total and items per page
    // check to make sure itemsPerPage != 0
    this.numberOfPages = Math.ceil(itemsTotal / (itemsPerPage || 10));
