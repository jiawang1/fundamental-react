...
export default class Tree extends Component {
  ...
  updateVisibility(selected) {
    return () => {
      const modifiedStates = this.state.iStates;
    ...
    const trees = treeData.map(row => {
      const parent = row.values.map((element, index) => {
        ...
        if (row.hasChildren & (row.values.indexOf(element) === 0)) {
          ...
          return (
            <div key={index} className="fd-tree__col fd-tree__col--control">
              <button className="fd-tree__control" aria-label="expand" aria-controls="inYUX852" onClick={this.updateVisibility(row.id)} aria-pressed={this.state.iStates[row.id]} />
              {element}
            </div>
          );
        }
      ...
      const displayLevel = `fd-tree__group fd-tree__group--sublevel-${depthLevel}`;
          ...
          <div className="fd-tree__row fd-tree__row--header">
            {headers.map((header, index) => {
              if (headers.indexOf(header) === 0) {
                return (
                  <div key={index} className="fd-tree__col fd-tree__col--control">
                    <button className="fd-tree__control " aria-label="expand" aria-pressed={this.state.expandAllClicked} onClick={e => this.openAllList(treeData, e)} />
                    {header}
                  </div>
                );
              }
